function V = CalculateVolume(L,si)
% Calculate volume defined by a surface si
%
%   V = L.CalculateVolume(si);
%
% Input:  - si is a surface index from 1 (endo) to L.nSurfaces (epi).
% Output: - V is volume in ml.
%
% Author: Avan Suinesiaputra - Auckland Bioengineering Institute 2011

V = 0;
if( isempty(L) ), return; end
if( si<1 || si>L.nSurfaces ), error('Invalid surface index.'); end

% get the enclosed volume
FV = L.GetClosedSurfaceTriangulation(si);
V = VolumeByTriangulation(FV.Faces,FV.Vertices);


% % % % % % % cog = mean(FV.Vertices);
% % % % % % % 
% % % % % % % % calculate volumes
% % % % % % % 
% % % % % % % % Volume of a patch is approximated by:
% % % % % % % %   V = | det([
% % % % % % % %               FV1 FV1 FV1 1
% % % % % % % %               FV2 FV2 FV2 1
% % % % % % % %               FV3 FV3 FV3 1
% % % % % % % %               C1  C2  C3  1
% % % % % % % %                              ]) |
% % % % % % % % where [C1,C2,C3] is the mean point of the patch [FV1,FV2,FV3].
% % % % % % % % So we can approximate the volume of the heart by sum of volumes of each
% % % % % % % % patch to the mean point. We assume that there is no hole in a heart.
% % % % % % % 
% % % % % % % V = zeros(size(FV.Faces,1),1);
% % % % % % % for i=1:size(FV.Faces,1)
% % % % % % %     V(i) = abs(det([[FV.Vertices(FV.Faces(i,:),:); cog] ones(4,1)]));
% % % % % % % end
% % % % % % % V = V ./ 6;
% % % % % % % 
% % % % % % % % total volume in ml
% % % % % % % V = sum(V) / 1000;
% % % % % % % end